<div class="group absolute bottom-5 right-4 z-10 flex flex-row items-center gap-3 p-3 text-lg">
    <div class="group relative" data-headlessui-state="open">
        <button
            ...
            wire:click="$emit('changeHeaderLock')"
        >
        ...
        @if ($isActive) 
            <div class="absolute bottom-full right-0 z-20 mb-2 w-full min-w-[175px] overflow-hidden rounded-md bg-white pb-1.5 pt-1 outline-none opacity-100 translate-y-0 text-base">
                <a class="flex p-3 items-center gap-3 transition-colors duration-200 cursor-pointer hover:text-blue-600">
                    <i class="fa-solid fa-circle-up"></i>
                    <span>Back to top</span></a>
                <a wire:click='changeLock' class="flex p-3 items-center gap-3 transition-colors duration-200 cursor-pointer hover:text-blue-400">
                    @if ($headerIsLocked)
